...
namespace conexao
{
    ...
    public partial class Form1 : Form
    {
        ...
        public static class Criptografia
        {
            ...
            public static string MD5(string cpf)
            {
                try
                {
                    System.Security.Cryptography.MD5 md5 = System.Security.Cryptography.MD5.Create();
                    byte[] inputBytes = System.Text.Encoding.ASCII.GetBytes(cpf);
        ...
        {
            string sql = "insert into alunos (nome,cpf,rg) values (@nome,@cpf,@rg)";
            ...
            comando.Parameters.Add("@cpf", MySqlDbType.VarChar).Value = Criptografia.MD5(textBox3.Text);

            comando.Parameters.Add("@rg", MySqlDbType.VarChar).Value = textBox4.Text;
        ...
        {
            string sql = "update alunos set nome=@nome,cpf=@cpf,rg=@rg where codigo= '"+textBox1.Text+"'";
            //enviando uma instrução em sql para a variavel com nome de sql
        ...
        private void consultar()
        {
            ...
            string sql = "select * from alunos ORDER BY CODIGO";//enviando uma instrução em sql para o banco
            ...
            if (leia.HasRows)//encontrou alguma coisa?
            {
                while (leia.Read())//enquanto o leia estiver encontrando dados ele vai exibir as informações no grid, registro por registro
                {
                    dataGridView1.Rows.Add(Convert.ToString(leia["codigo"]), Convert.ToString(leia["nome"]),
                        Convert.ToString(leia["cpf"]), Convert.ToString(leia["rg"]));
                }
            }
            else
            {
        ...
        {
            string sql = "delete from alunos where codigo= '" + textBox1.Text + "'";
            //enviando uma instrução em sql para a variavel com nome de sql
